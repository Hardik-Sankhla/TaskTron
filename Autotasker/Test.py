...
# Import Wikipedia search functions
from wikipedia_search import search_wikipedia, suggest_wikipedia, summary_wikipedia, get_page_details
...
def execute_command(command):
    ...
    command_keywords = {
        ...
        "search about": lambda query: search_wikipedia(query),
        "suggest": lambda query: suggest_wikipedia(query),
        "summary": lambda query: summary_wikipedia(query),
        "page": lambda query: get_page_details(query),
        "google search": lambda query: google_search(query)
        
    }

    for keyword, function in command_keywords.items():
        if keyword in command:
            # Extract query part of the command
            query = command.replace(keyword, "").strip()
            result = function(query)
            return result if result else f"Executing {keyword} with query '{query}'"
